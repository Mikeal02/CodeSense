  ...
  return (
    <section ref={sectionRef} className="py-20 sm:py-28 relative">
      ...
      <div className="container mx-auto px-4 sm:px-6 relative">
        ...
        >
          <p className="text-center text-[10px] uppercase tracking-[0.25em] text-muted-foreground/50 mb-8">
          ...
          <div className="relative overflow-hidden">
            ...
            >
              {[...logos, ...logos, ...logos].map((logo, i) => (
                <span key={i} className="text-sm font-medium text-muted-foreground/20 whitespace-nowrap select-none tracking-wide">
                  {logo}
                </span>
              ))}
            </motion.div>
        ...
        {/* Stats */}
        <div ref={statsRef} className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-16 sm:mb-20">
          {stats.map((stat, i) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 16 }}
              animate={statsVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
              transition={{ delay: i * 0.08, duration: 0.5 }}
              className="text-center p-6 rounded-xl border border-border/50 bg-card/40"
            >
              <div className="w-10 h-10 mx-auto mb-3 rounded-lg bg-secondary flex items-center justify-center">
                <stat.icon className="w-5 h-5 text-primary" />
              </div>
              <div className="text-2xl sm:text-3xl font-bold text-foreground mb-1 font-mono tracking-tight">
          ...
          <div className="space-y-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4">
            {testimonials.map((t, i) => (
              <motion.div
                key={t.name}
                initial={{ opacity: 0, y: 16 }}
                animate={testimonialsVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 16 }}
                transition={{ delay: i * 0.1, duration: 0.5 }}
                className="p-6 rounded-xl border border-border/50 bg-card/40"
              >
        ...
        >
          <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-xl border border-border bg-card/60">
